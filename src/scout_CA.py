...
class ScoutCollisionAvoidance:
    def __init__(self):
        ...
        print("left:{}".format(self.guidAngleLeft))
        self.guidAngleRight = (MaxRightDegree / 180.0 * math.pi)
        print("right:{}".format(self.guidAngleRight))
    ...
    def process(self):
        ...
        if self.len_obj > 0:
            ...
            if (maxIdx == 0):
                ...
                return tmpResultMsg
            tmpIdx = maxIdx - 1
            tmpDist1 = np.sqrt(focusObjs[tmpIdx].maxCircleRadius**2 + focusObjs[tmpIdx].distFromEgo**2)
            tmpDist2 = np.sqrt(focusObjs[tmpIdx-1].maxCircleRadius**2 + focusObjs[tmpIdx-1].distFromEgo**2)
            ...
            angleGapC = np.arccos(tmpNum/tmpDen) - (focusObjs[tmpIdx].marginAngleLeft)
    ...
    def filtering(self):
        ...
        for obj in tmpNpArr:
            ...
            obj.distFromEgo = np.linalg.norm([(obj.posX - self.egoPosX), (obj.posY - self.egoPosY)], ord=2)
            tmpFilterList.append(obj.angleFromEgo)
        # for x in tmpFilterList:
        #     print( x / math.pi * 180)

        # print("tmpFilterList:{}".format(tmpFilterList) )
        tmpFilterArr = np.asarray(tmpFilterList)
        tmpMask = np.logical_and((tmpFilterArr < self.guidAngleLeft), (tmpFilterArr > self.guidAngleRight))
        # print(tmpMask)
        filteredDatas = tmpNpArr[tmpMask]
        filteredAngles = tmpFilterArr[tmpMask]

        # for x in filteredAngles:
        #     print( x / math.pi * 180)
        sortedDatas = []
